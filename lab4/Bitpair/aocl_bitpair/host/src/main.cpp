		...
		switch(c){
			...
			case '?':
				if(optopt == 'i' || optopt == 'o')
					...
					printf("Unknown characters.  Usage is ./bitpair -i INFILE -o OUTFILE\
						[-t] [-v]\n\tUsing -t the -i and -o flags are not required\n");
				}
				return 1;
		}
	}
...
bool init_opencl() {
  	...
  	std::string binary_file = getBoardBinaryFile("bitpair", *device);
  	//printf("Using AOCX: %s\n", binary_file.c_str());
...
void init_problem(char *input_file) {
	...
	if(test){
		N = 10;
		input_a = (short*) malloc(sizeof(short) * N);
		input_b = (short*) malloc(sizeof(short) * N);
		...
		fread(input_a, sizeof(short), N, INFILE);
		fread(input_b, sizeof(short), N, INFILE);
...
void run(char *output_file) {
	...
	status = clEnqueueWriteBuffer(queue, input_a_buf, CL_FALSE,
			0, sizeof(short) * N, input_a, 0, NULL, &write_event);
	...
	status = clEnqueueWriteBuffer(queue, input_b_buf, CL_FALSE,
			0, sizeof(short) * N, input_b, 0, NULL, &write_event);
	checkError(status, "Failed to write input buffer");
	...
	status = clEnqueueReadBuffer(queue, output_buf, CL_FALSE,
       		0, sizeof(int)*N, output, 1, &kernel_event, &finish_event);
